...
import pytest
from src.agents.sales.segmentation_ad_targeting_agent import (
...
def test_segmentation_targeting(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.tools.ad_tool.AdTool.create_facebook_campaign",
        lambda self, n, a, b: {"campaign_id": "fb1"},
    )
    monkeypatch.setattr(
        "src.tools.ad_tool.AdTool.create_google_campaign",
    ...
    payload = SegmentationEvent(
        segments=[
            {
                "name": "A",
                "criteria": {"facebook_audiences": [1], "google_keywords": ["x"]},
            }
        ],
